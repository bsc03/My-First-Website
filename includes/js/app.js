...
var validate = function (e) {
    var fields = document.querySelectorAll('.flex-left textarea, . flex-left input[type="text"]');
    var regEx;
    var removeSpan;
    var par;
    var check = false;
    ...
    for (var i = 0; i < fields.length; i++) {
        if (fields[i].value === "") {
            if (fields[i].nextElementSibling.classList.contains('error')) {
                ...
                fields[i].nextElementSibling.innerHTML = "Oops!! " + fields[i].placeholder + " is required.?";
                fields[i].computedStyleMap.boxShadow = "0 0 2px 1px #cc0001"
                check = false;
            ...
            // check if message and name value contain valid characters
            if (fields[i].id !== 'email' && fields[i].id !== 'phone') {
                val = isValidChar(fields[i]);
                if (val === false) {
                    fields[i].nextElementSibling.innerHTML = "Oops!! are you try to hack.";
                    fields[i].computedStyleMap.boxShadow = "0 0 2px 1px #cc0001";
                    check = false;
                    errArr.push(fields[i]);
                } else {
                    fields[i].nextElementSibling.innerHTML = "";
                    fields[i].computedStyleMap.boxShadow = "none";
                    check = true;
                }
            }
            if(fields[i].id === 'phone'){
                ...
                if(val=== false){
                    fields[i].nextElementSibling.innerHTML = "Oops!! Your phone number is not valid?";
                    fields[i].computedStyleMap.boxShadow = "0 0 2px 1px #cc0001";
                    check=false;
                    errArr.push(fields[i]);
                }else{
            ...
            if (fields[i].id === 'email') {
                ...
                if(val === false) {
                    fields[i].nextElementSibling.innerHTML = "Oops!! Your email address is not valid?";
